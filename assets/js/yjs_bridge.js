  ...
  const handlers = {
    createDoc({ clientId, undo = false }) {
      ...
      const undoManager = undo
        ? (() => {
            const um = new UndoManager(objects, { captureTimeout: 500 });
    ...
    applyUpdate({ clientId, updateB64, origin }) {
      const entry = need(clientId);
      applyUpdate(entry.doc, b64ToU8(updateB64), origin ?? 'remote');
      return {};
    },
    ...
    createObject({ clientId, objectId, kind, box, src }) {
      ...
      entry.doc.transact(() => {
        ...
        if (kind === 'title') obj.set('text', new YText());
    ...
    projection({ clientId }) {
      ...
      return {
        ...
        objects: out,
        stateVectorB64: u8ToB64(encodeStateVector(entry.doc)),
      };
    },

    encodeStateAsUpdate({ clientId }) {
      return { updateB64: u8ToB64(encodeStateAsUpdate(need(clientId).doc)) };
    },
    ...
    decodeUpdate({ updateB64 }) {
      const decoded = decodeUpdate(b64ToU8(updateB64));
    ...
    pendingStructCount({ clientId }) {
      ...
      const decoded = decodeUpdateV2(pending.update);
    ...
    yjsVersion() {
      return { hasYText: typeof YText === 'function', hasYMap: typeof YMap === 'function' };
    },
  };
